...
def main() -> None:
    ...
    dist_index = assets_dir / "dist" / "index.html"
    index_file = dist_index if dist_index.exists() else assets_dir / "index.html"

    if not index_file.exists():
        raise SystemExit(
            "Front-end bundle missing. Run `npm run build` inside frontend or keep frontend/index.html in place."
        )
